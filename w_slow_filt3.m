...
L = 2^nextpow2(param.daq.LFdaq.pts);
freq = linspace(0,Fs,L);
Fs_us = param.daq.HFdaq.pulseRepRate_Hz;


L_us = 2^nextpow2(param.daq.HFdaq.NoBurstTriggers);
 ...
 %   fprintf('Filtering 4D data\n')
    ...
    for i = 1:size(HF,1)
        for j = 1:size(HF,2)
            for k = 1:size(HF,3)
                if param.medfilt
                    y(i,j,k,:) = medfilt1(real(ifft(fft(squeeze(HF(i,j,k,:))).*H',L)),3);
                else
                    y(i,j,k,:) = real(ifft(fft(squeeze(HF(i,j,k,:)),L_us).*H,size(HF,4)));
                end
            end
        end
